...
export async function GET(request: Request) {
  ...
  const sort = url.searchParams.get('sort') === 'reward' ? 'reward' : url.searchParams.get('sort') === 'popular' ? 'popular' : 'recent';
  ...
  const doams = await db.doAm.findMany({
    ...
    include: { creator: { include: { profile: true } }, location: true, _count: { select: { likes: true, comments: true, applications: { where: { status: 'INTERESTED' } } } } },
    orderBy: sort === 'reward' ? { reward: 'desc' } : sort === 'popular' ? { likes: { _count: 'desc' } } : { createdAt: 'desc' },
    take: take + 1,
...
export async function POST(request: Request) {
  ...
  try { return Response.json(await createDoAm(session.user.id, await request.json()), { status:201 }); }
  catch (error) { return Response.json({ error:{ code:'VALIDATION_ERROR', message:error instanceof Error ? error.message : 'Invalid request.' } }, { status:400 }); }
}
